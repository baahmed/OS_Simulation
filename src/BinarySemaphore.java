public class BinarySemaphore {
	/*
	 * use mutex if this resource is used by a process and released by the same
	 * process. use binary semaphores for conditioned use of the resource. Since
	 * both are allowed, I'm using this
	 */

	private volatile int value; // the value of the mutex instance - free (1) or not free(0)?
	public volatile Process freeThisProcess = null; // this is the process that will be freed - according to the
													// scheduler
...
<<<<<<< HEAD
	...
	public void semWait(Process p, String rsc) {
		System.out.println("should be running" + p.getProcessID() + " " + Process.getProcessState(p));
...
>>>>>>> f50a2154a13d1bc05376cea4d849f170d3c56ce6
			...
			p.suspend();
			// semWait keeps the process busy as long as it's not allowed to claim the
			// resource.
			while (value == 0) {
...
<<<<<<< HEAD
			...
			value = 0; // take the resource, the process is now ready with it until it's called to
						// execute.
...
<<<<<<< HEAD
	public void semPost(int id, String rsc) {
		...
		// which process is being freed? we need to know which queue to take a process
		// from
		switch (rsc) {
		case "read":
			freeProcess = (!(OperatingSystem.getBlockedReadQueue()).isEmpty())
					? (Process) OperatingSystem.getBlockedReadQueue().poll()
					: null;
			break;
		case "write":
			freeProcess = (!(OperatingSystem.getBlockedWriteQueue()).isEmpty())
					? (Process) OperatingSystem.getBlockedWriteQueue().poll()
					: null;
			break;
		case "print":
			freeProcess = (!(OperatingSystem.getBlockedPrintQueue()).isEmpty())
					? (Process) OperatingSystem.getBlockedPrintQueue().poll()
					: null;
			break;
		case "input":
			freeProcess = (!(OperatingSystem.getBlockedInputQueue().isEmpty()))
					? (Process) OperatingSystem.getBlockedInputQueue().poll()
					: null;
			break;
		default:
			System.out.println("unsupported kernel command");
		...
		if (!(freeProcess == null)) {
			...
			setFree(freeProcess); // do not set semaphore to 1 - just free a blocked process (it took the rsc
									// already)
			// TODO: comment next line
			// System.out.println("process " + freeProcess.getProcessID() + " is now
			// ready");
